      ...
      if (company?.expand?.management_waits[0]?.id.length >= 1) {
        const response = await client
          ...
          .getOne(company?.expand?.management_waits[0]?.id, {
            expand: 'user_waits',
            fields:
              'expand.user_waits.id, expand.user_waits.user_phone_number, expand.user_waits.admission_status, expand.user_waits.adult_persons, expand.user_waits.child_persons, expand.user_waits.created',
          });
